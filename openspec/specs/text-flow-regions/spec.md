...
`XTextFormatter` lays text out around things the caller has put in the block. The caller supplies the
obstacles, because the caller drew whatever is standing there; the formatter narrows the lines whose
band each one stands in, and moves a line down past anything that leaves it no room at all.

The geometry is asked one question per line — *at this band, which horizontal runs are free?* — and
answers with a **set** of runs rather than a single span. That is more general than either reference
implementation read before building it: iText7 coalesces every left float into one boundary and every
right into another and clips each line between them, so it can never describe two runs; QuestPDF
hands a single width to Skia's shaper and has no per-line seam at all. The set is carried anyway so
that taking one run is a decision the layout loop makes rather than a property of the type — which is
what keeps an irregular obstacle a new implementation instead of a redesign.

A band is a **line box** and not a baseline, because a line whose baseline clears an obstacle can
still have ascenders reaching into it.

Deliberately left out, and each of them cheap to add later rather than blocked:

- **Contour obstacles.** A rectangle is the only implementation that ships. An ellipse, a polygon and
  an `XGraphicsPath` are further implementations of the obstacle interface — flatten, intersect the
  band with the edges, pair the crossings — not changes to anything else.
- **More than one run per line.** The geometry reports every free run and the loop fills the widest.
  Filling several means one logical line spanning them, which justification, alignment and truncation
  all assume never happens.
- **Per-side padding.** One distance, on the obstacle. MigraDoc carries four and they earn their
  keep; growing this to four changes the rectangle obstacle and nothing else.
- **Obstacles that move the text down.** An obstacle narrows lines. It never pushes the block's top
  down or grows the layout rectangle.
- **Obstacles in page coordinates.** See the requirement on coordinates below: layout runs unrotated
  and rotation is applied to the drawing surface afterwards, so a page-space obstacle under arbitrary
  rotation would inverse-rotate a rectangle into a quad and drag the polygon work onto the critical
  path in order to support rectangles.

This rests on `variable-line-measure`, which is what lets the room available to a line depend on
where that line sits, and it shares `drop-cap`'s reservation mechanism — the cap is an obstacle like
any other, and the only one the formatter makes for itself.

The MigraDoc equivalent is `shape-side-wrap`, which reaches the same idea through `Area` rather than
through the formatter. The two remain separate implementations on purpose: MigraDoc is handed a shape
in a document tree and works out where it lands, and this is handed a rectangle by a caller who drew
it and therefore knows. Worth noting the asymmetry that leaves — `ObstructedArea` answers with one
rectangle and cannot express two runs, where this can.
