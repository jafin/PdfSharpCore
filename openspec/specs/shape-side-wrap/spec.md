...
Text flowing beside a shape rather than stopping above it and starting again below it. A sidebar, a
pull quote and a picture the copy closes around are the same arrangement with a different value on
`WrapFormat.Style`, and MigraDoc could express none of them: `WrapStyle` offered `TopBottom`, `None`
and `Through`, which place a shape between the elements around it, ignore it, or let the text run
straight over it.

The whole of it is one `Area` implementation. `Area.GetFittingRect(y, height)` is already asked once
per line by `ParagraphRenderer`, and every other thing the paragraph loop does - breaking lines,
justifying, aligning, truncating - is already expressed in terms of what it answers. An area that
answers with a narrower rectangle where an obstacle overlaps therefore makes text flow around that
obstacle with no change to the line-breaking loop.

The side is expressed in the obstacle rather than in the area: a shape the text runs down the left of
blocks everything from its own left edge to the right edge of the area, so the only clear span left
is the one the caller asked for. `ObstructedArea` knows nothing about sides.

Not covered, and deliberately: contour wrapping (the shape is tested as a box), a shape spanning a
page break, wrapping beside a table, and overlap resolution between two floating shapes. A band is
given its widest clear span rather than every clear span, because text that hops across a pull quote
and back mid-line is unreadable whatever the geometry says - which is why `Both` and `Largest` lay
out alike today.

`XTextFormatter` is a drawing surface with no notion of a shape and is unaffected. See
`variable-line-measure` for the same idea in that engine.
