...
Changing the size, shape or orientation of a PDF page that already carries content, in the document
that holds it, with what is drawn on the page scaled into the new size rather than cropped by it,
and with the annotations of the page and the link destinations that point at it carried across.

Also covers the behaviour of the existing `PdfPage.Size`, `Width` and `Height` setters on such a
page: they write a new media box and nothing else, which crops rather than resizes, and so refuse
a page that has content on it.

Design notes, the mechanism and what was deliberately left out are in `docs/specs/page-resize.md`.
