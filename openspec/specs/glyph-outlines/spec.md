...
Turning text into path geometry: `XGraphicsPath.AddString` adds the outlines of a string's glyphs to
a path as lines and Bézier curves, laid out as `XGraphics.DrawString` would lay the same string out
for the same point or rectangle and format. The resulting path is a path like any other, so the
glyphs can be filled with a gradient, used as a clip, or widened — none of which drawing text can
do.

Also covers the seam the outlines arrive through. The core package carries no font-rasterizing
dependency, so outlines come from a registered provider, in the same manner as
`GlobalFontSettings.FontResolver` and `ImageSource.ImageSourceImpl`, and an unregistered provider is
reported rather than ignored. A caller who only wants text stroked or filled needs none of this, and
