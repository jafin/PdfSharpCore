...
Artwork that runs off the edge of the paper. `PdfPage.TrimMargins` makes the sheet larger than the
page and puts the drawing origin on the page's corner, so a caller reaches into the bleed with
negative coordinates and writes exactly the same code as on an untrimmed page everywhere else.
Saving writes the five page boxes a printer reads: the sheet, the bleed the ink may run to, and the
trim where the guillotine cuts.

Also covers `PdfPage.MarkMargins`, which is the room left outside the bleed for printer's marks and
the switch that decides whether the eight standard crop marks are drawn into it, and the route by
which a MigraDoc document is laid out onto a trimmed page.

Every part of this existed and worked before it was specified — `TrimMargins` arrived with the
upstream library — but it had no test, no demonstration and no documentation, so nobody could find
it and nothing protected it. Writing the tests found three defects in it, all recorded here as the
behaviour they now have rather than the behaviour they had.
