...
- [ ] 1.1 Write the failing tests first, in `PdfSharpCore.Test/Rendering/`: a table whose second row
...
- [ ] 1.2 In `MigraDocCore.Rendering/MigraDoc.Rendering/TableRenderer.cs`, after
...
- [ ] 1.3 Cover the scenarios in `specs/repeating-table-headings/spec.md` that must keep working:
...
- [ ] 1.4 Add a line to the release notes. The throw is a behaviour change, however narrow.

## 2. Baseline alignment with a rectangle that has a height

- [ ] 2.1 **Answer the open question before writing anything**: read the placement code below
...
- [ ] 2.2 Remove the guard at `XGraphics.cs:1295` and the matching one at `XGraphicsPath.cs:391`.
      Both, in the same commit: they are separate code that must agree.
- [ ] 2.3 Test that the same string with the same `BaseLine` format lands in the same place for
      rectangles of height 0, 20 and 200 — this is the requirement, and it is what catches a
      height that leaks into the arithmetic.
- [ ] 2.4 Test `XStringFormats.Default` into an ordinary rectangle, named for the trap it is:
      the format a caller reaches for when not thinking about formats used to throw on the most
      natural overload.
- [ ] 2.5 Test `BaseLineRight` and `BaseLineCenter` with a width, confirming horizontal alignment
      still applies and only the vertical dimension is being ignored.

## 3. Gradients that honour alpha

- [ ] 3.1 Pin the current output first: save a document of opaque gradients, keep the bytes, and
...
- [ ] 3.2 In `PdfShading`, extract the function-and-coords construction so the same geometry can be
...
- [ ] 3.3 Build the alpha shading: same `/Coords`, same `/FunctionType 2`, `/ColorSpace /DeviceGray`,
      `/C0` and `/C1` as the source colours' alpha in 0..1.
- [ ] 3.4 Build the luminosity group: a `PdfFormXObject` with
...
- [ ] 3.5 Wire it together: a `PdfSoftMask` with `/S /Luminosity` and `/G` the form, on a
...
- [ ] 3.6 Gate the whole branch on `either colour's A < 255`. Re-run 3.1 and confirm the opaque
      bytes are identical.
- [ ] 3.7 Test the appearance: transparent-to-opaque over a filled rectangle, half-alpha over white
...
- [ ] 3.8 Test the structure as well as the pixels: assert the saved document has an `ExtGState`
      ...
      contract.
- [ ] 3.9 Test that the graphics state is left as found — an opaque black rectangle drawn after a
      masked gradient is fully opaque, and two gradients with different ramps on one page do not
      mask each other.

## 4. Glyph outlines and `AddString`

- [ ] 4.1 Define `IGlyphOutlineProvider` and `XGlyphOutline` in `PdfSharpCore/Fonts/`, and the
...
- [ ] 4.2 Test the unregistered case first — `AddString` throws with a message naming the property
...
- [ ] 4.3 Implement `SkiaGlyphOutlineProvider` in `PdfSharpCore.Skia`: font bytes from the
...
- [ ] 4.4 Implement `ImageSharpGlyphOutlineProvider` in `PdfSharpCore.ImageSharp` against
      SixLabors.Fonts' `IGlyphRenderer`, whose callbacks are already move/line/quadratic/cubic/end.
- [ ] 4.5 Implement both `XGraphicsPath.AddString` overloads on top of the seam, feeding
...
- [ ] 4.6 Apply the group 2 rule here too: a `BaseLine` format with a rectangle of any height
      places the baseline on the top edge.
- [ ] 4.7 Test that the path is non-empty and its bounds approximate `MeasureString`, for a
...
- [ ] 4.8 Test the three things a path can do that `DrawString` cannot: filled with a gradient,
      used as a clip with a photograph drawn through it, and an empty string adding nothing and
      throwing nothing.
- [ ] 4.9 Answer the open question about glyphs with no outline — bitmap-only glyphs, `.notdef` —
      by checking what `DrawString` does with the same character, then match it and write a test
      recording the choice.
- [ ] 4.10 XML-document `AddString` naming `DrawString` with a pen as the way to stroke text
...
- [ ] 5.1 `SampleApp/Demos/MagazineDemo.cs` builds its scrim from 140 translucent bands and strokes
...
- [ ] 5.2 Update `Shows` on `MagazineDemo` — it currently advertises the workarounds.
- [ ] 5.3 Update `docs/specs/demonstration-app.md`: the four gaps move from "what the library does
      not do" to a note that they were found here and fixed under this change.
- [ ] 5.4 Re-run the demo smoke tests. `Magazine` declares two pages; changing how it draws must
      not change how many pages it draws, and the test will say so if it does.

## 6. Close out

- [ ] 6.1 `./ci-build.ps1` clean and `dotnet test` green on both target frameworks.
- [ ] 6.2 Rasterize the affected demo pages and look at them. Every one of these four gaps was found
      by looking at a page and none by a test; the same standard applies to the fixes.
- [ ] 6.3 Run the demo app on Linux, or check CI does, since gradients with soft masks and glyph
