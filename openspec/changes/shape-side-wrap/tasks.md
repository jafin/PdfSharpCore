...
- [x] 4.1 Return the side values from `ShapeRenderer.GetFloating`, which can return only `TopBottom`
      or `None` today.
- [x] 4.2 Teach `TopDownFormatter` the difference. Three lines ask `Floating != Floating.None`; a
...
- [x] 4.3 Give the obstacle the shape's rectangle grown by the four `WrapFormat` distances, so that
      all four start meaning something. Two of them have never meant anything.
- [x] 4.4 Fall back to `TopBottom` for a shape too tall for its area or one that would span a page
      break, rather than producing an obstacle that outlives the area holding it.
- [x] 4.5 Test the requirements in `specs/shape-side-wrap/spec.md` that catch silent wrongness: no
      line drawn across the shape, every word present exactly once, and the lines above and below
      running the full measure.
- [x] 4.6 Test justified text beside an obstacle by reading the drawn positions. Justification
...
- [x] 4.7 Re-run 1.1. A document with no side wrap must still be byte-identical. **Identical**, all
      10 — but only after the pin caught the rendering phase carrying a formatting-phase rect into
      a table cell, which put the cell's text back where the cell used to be. That is the regression
      this group of tasks exists to catch, and it caught it.
